...
const DESCRIPTION = `${PREFIX}search searches for images on database.
...
> ${PREFIX}search "query to search" [--OptionalFields] [--OptionalOptions]
...
**Search with Fields**:
\`\`\`
${PREFIX}search rowi --folder cmx_20 --hobby mangap
\`\`\`
...
\`\`\`
${PREFIX}search rowi --channel #general-chat
\`\`\`
`
...
export default async function searchCommand(message: Message, cmd: string) {
    ...
    if (!args._.length) {
        await message.channel.send(
            `Ns query search detected from your message request. Please type only \`${PREFIX}search\` in the text box for query info.\nIf you are trying to get images by folder, please use \`${PREFIX}folder\``
        )
    ...
    let page = parseInt(args.page) - 1 || 0
    if (page < 0) {
        page = 0
    }

    let limit = parseInt(args.limit) || 5
    if (limit > 5) {
        limit = 5
    }
